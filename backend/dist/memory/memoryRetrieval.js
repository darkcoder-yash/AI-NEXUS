...
export class MemoryRetrieval {
     ...
     */
    static async getRelevantContext(query, userId, topK = 5) {
        try {
            const queryEmbedding = await embeddingService.generateEmbedding(query);
            // 1. Fetch results filtered by userId at the database level
            // FIXED: Added userId as second argument
            const results = await memoryClient.searchMemory(queryEmbedding, userId, topK * 2);
