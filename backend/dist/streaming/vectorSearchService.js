...
export class VectorSearchService {
     ...
     */
    async search(userId, query, currentConversationId) {
        const queryEmbedding = await this.embeddingService.generateEmbedding(query);
        // 1. Initial vector search (similarity only)
        const results = await this.repository.searchSimilarMemories(userId, queryEmbedding, currentConversationId);
    ...
    applyHybridRanking(results) {
        ...
        const ranked = results.map(memory => {
            ...
            const recencyScore = Math.exp((-Math.log(2) * diffDays) / MemoryConfig.RECENCY_HALFLIFE_DAYS);
            const hybridScore = (similarity * MemoryConfig.WEIGHT_SIMILARITY) +
