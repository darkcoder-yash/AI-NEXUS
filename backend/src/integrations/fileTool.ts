...
const initializeSandbox = async () => {
  ...
  } catch (err) { console.error('Failed to initialize sandbox directory.'); }
};
initializeSandbox();
...
export const FileTool = {
  read_file: {
    ...
    execute: async ({ fileName }: { fileName: string }, userId: string) => {
  ...
  write_file: {
    ...
    execute: async ({ fileName, content }: { fileName: string, content: string }, userId: string) => {
      const safePath = validatePath(fileName);
 ...
 */
function validatePath(fileName: string): string {
