...
export const CalendarTool = {
  ...
  create_event: {
    ...
    execute: async (args: { summary: string, start: string, end: string, location?: string, description?: string }, userId: string) => {
      const accessToken = await OAuthManager.getValidAccessToken(userId);
  ...
  delete_event: {
    ...
    execute: async ({ eventId }: { eventId: string }, userId: string) => {
      const accessToken = await OAuthManager.getValidAccessToken(userId);
