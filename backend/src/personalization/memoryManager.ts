import { StructuredMemory, UserProfile, MemoryType } from './personalizationTypes.js';
...
export class MemoryManager {
   ...
   */
  async saveStructuredMemory(userId: string, type: MemoryType, content: string, score: number) {
    ...
    if (similarMemories.length > 0) {
      ...
      await this.db.query(
        `UPDATE structured_memories 
         ...
         WHERE id = $4`,
        [content, embedding, score, existing.id]
      );
    } else {
      // 2. Insert new structured memory
      await this.db.query(
        `INSERT INTO structured_memories (user_id, type, content, importance_score, embedding)
         VALUES ($1, $2, $3, $4, $5)`,
        [userId, type, content, score, embedding]
      );
    }
  }
  ...
  async deleteMemory(userId: string, memoryId: string) {
    await this.db.query(
      `DELETE FROM structured_memories WHERE id = $1 AND user_id = $2`,
      [memoryId, userId]
    );
  }
}
