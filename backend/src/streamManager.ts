...
export class StreamManager {
  ...
  private onEvent: (type: WebSocketEventTypes, payload: any) => void;
   ...
   */
  async processUserUtterance(text: string) {
    ...
    try {
      ...
      await this.agent.processMessage(text, async (type, payload) => {
        // Stop if cancelled
        if (this.currentResponseController?.signal.aborted) return;
