...
import { Memory, MemorySchema, SearchResult } from './memoryTypes.js';
...
export class MemoryClient {
   ...
   */
  async saveMemory(memory: Omit<Memory, 'id' | 'created_at'>): Promise<Memory> {
    try {
   ...
   */
  async searchMemory(queryEmbedding: number[], userId: string, topK: number = 5): Promise<SearchResult[]> {
    try {
