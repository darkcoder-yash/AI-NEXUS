...
export class MemoryScoring {
  ...
  static estimateSentiment(text: string): 'positive' | 'neutral' | 'negative' {
  ...
  static detectTopic(text: string): string {
    const topics = {
  ...
  static shouldStore(userPrompt: string, aiResponse: string): boolean {
